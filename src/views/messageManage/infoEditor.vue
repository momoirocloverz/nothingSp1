    ...
    <div class="u-info-editor">
        <div class="subCon">
            <el-breadcrumb separator-class="el-icon-arrow-right" class="u-breadcrumb">
                <el-breadcrumb-item :to="{ path: '/infoManage' }">消息管理</el-breadcrumb-item>
            ...
            <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="u-ruleForm">
                <el-form-item label="标题" prop="title">
                    <el-input v-model="ruleForm.title" maxlength="12" placeholder="添加标题"></el-input>
                </el-form-item>
                <el-form-item label="标题简介" prop="source">
                    <el-input v-model="ruleForm.source" maxlength="150" placeholder="添加标题简介"></el-input>
                </el-form-item>
                ...
                <!--<el-form-item label="消息简介" prop="description">-->
                <!--<el-input v-model="ruleForm.description" maxlength="140"  placeholder="请输入140字以内,如不填默认显示提取消息内容前140字" ></el-input>-->
                <!--</el-form-item>-->
            </el-form>
            ...
            <div class="u-button-group">
                <!--<el-button type="primary" @click="handleSaveAndUp({status:0})" style="margin-right:40px;">保存</el-button>-->
                <el-button type="primary" @click="handleSaveAndUp()">保存</el-button>
                <div class="u-ready" @click="handleShowReady">预览</div>
            </div>
            <el-dialog  title="预览"   :visible.sync="show"  width="600px" class="u-dialog-wrapper">
                <div class="u-ready-wrapper">
                    ...
                    <div class="u-detail-content ql-editor" v-html="ruleForm.content"></div>
                </div>
    ...
    export default {
        ...
        data() {
            ...
            return {
                ...
                rules: {
                    ...
                    source: [{
                        required: true, message: '标题简介不能为空', trigger: 'blur'
                    }],
                    content: [{
        ...
        methods: {
            ...
            handleIsCreate() {
                ...
                if (type === '0') {
                    this.textSecond = '添加消息';
                } else {
                    this.textSecond = '编辑消息';
                    this.handleGetEditorInfo({id});
                }
            },
            handleGetEditorInfo({id}) {
                this.Apis.infoManageApi.getEditorDetail({id}).then(res => {
                    this.ruleForm.source = res.data.description;
            ...
            handleSave() {
                this.$confirm('确认发布？发布后将对所有零工发送push提醒?', `${this.ruleForm.title}`, {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    this.handleSaveAndUp()
                }).catch(() => {
                    this.$message({
                        type: 'info',
                        message: '已取消发布'
            ...
            handleSaveAndUp() {
                ...
                this.$refs['ruleForm'].validate((valid) => {
                    if (valid) {
                        if (id) {
                            this.Apis.infoManageApi.systemMsgUpdate({...this.ruleForm, id, description:this.ruleForm.source}).then(res => {
                                this.$router.replace({name: "messageManage"});
                            })
                        } else {
                            this.Apis.infoManageApi.systemMsgAdd({...this.ruleForm, description:this.ruleForm.source}).then(res => {
                                this.$router.replace({name: "messageManage"});
                            })
                        }
                    } else {
